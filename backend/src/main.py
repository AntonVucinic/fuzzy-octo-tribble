...
async def upload_text(law: Annotated[str, Body(media_type="text/plain")]) -> Any:
    ...
    return json.loads(response.to_dict()['candidates'][0]['content']['parts'][0]['text'])  # pyright: ignore[reportIndexIssue]


@app.post("/tax-info/file", response_model=Schema)
async def upload_file(law: UploadFile) -> Any:
    file = genai.upload_file(law.file, mime_type="application/pdf") # pyright: ignore[reportArgumentType]
